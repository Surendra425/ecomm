@extends('layouts.app')
@section('title') Seller Dashboard @endsection
@section('css')

@endsection
@section('content')
        <div class="container-fluid" id="dashboardBanner" style="background-image:url('{{ url('doc/store_banner_image/'.((isset($store->banner_image) && $store->banner_image != "")?$store->banner_image:"store_banner.png")) }}')">
            <div class="full-overlay">
                <div class="container" >
                    ...
                    <span class="textDashboard">
                        ...
                        <a class="fl_count"><span>({{ count($store->follower) }})</span></a>
                    </span>
                    </div>
                        ...
                        <div class="search-data">
                            <div class="col-xs-12">
                                <div class="search-box">
                                    ...
                                    <form id="searchKey" method="get" action="{{url(route('searchStoreProduct',['store' => $store->store_slug]))}}">
                                        <div class="input-group stylish-input-group">
                                            <input type="text" class="form-control required"  placeholder="Search in Store" name="keyword" id="srch-term3" required="required">
                                            <div class="hidden base base-width" id="search-div3">
                                                <ul id="search-parm3"></ul>
                                            </div>
                            ...
                            <div class="col-xs-12" id="deleveryDetails">
                                <ul class="list-inline">
                                    <li class="col-xs-2">
                                        ...
                                        @if(!empty($shipping))
                                            <span class="event-text" id="form">{{isset($shipping->from) && !empty($shipping->from) ? $shipping->from : '0'}}-{{isset($shipping->to) && !empty($shipping->to) ? $shipping->to : '0'}} {{isset($shipping->time) && !empty($shipping->time) ? $shipping->time : 'Days'}}</span>
                                        @else
                                            @if(empty($address))
                    ...
                    <div class="swiper-wrapper">
                        @foreach($category as $item)
                            <div class="swiper-slide">
                                <div class="item">
                                    <a href="{{ url(route('showSellerCategory',['categorySlug'=>$item->vendor_category_slug])) }}">
