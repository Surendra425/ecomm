                    ...
                    @foreach($stores as $k => $store)
                        <div class="col-lg-2 col-md-4 col-sm-4 col-xs-6 bestSellers">
                            <a href="{{ url(route('sellerDetail',['storeSlug'=>$store['store_slug']])) }}">
                                ...
                                <img alt="{{ $store['store_name'] }}" class="img-responsive" src="{{ ($store['store_image'] != "")? url('doc/store_image/').'/'.$store['store_image']:url('assets/app/media/img/no_store_image_100.png') }}">
                            </a>
                            <p class="card-3-text">
                                <a href="{{ url(route('sellerDetail',['storeSlug'=>$store['store_slug']])) }}">
                                    {{ $store['store_name'] }}
...
@section('js')
    <script>
        $(document).ready(function(){
            var url  = "{{url(route('showCategoryProducts'))}}";
            var page = 2;
            ...
            $(window).scroll(function () {
                ...
                if($(window).scrollTop() + $(window).height() > $(document).height() - 100) {
                    if (lastPage >= page) {
                        var data = collection = {'page': page, 'category_id':{{$category->id}}}
