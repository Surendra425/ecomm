...
//echo $selected_address_id; $user_address = UserAddress::find($selected_address_id);
        ...
        if ($order->save()){
           ...
           // die;
            if($request->payment_type == 'KNet'){
                ...
                //dd($order);die;
                return $this->knetPay($request, $orders);
            }
            else if($request->payment_type == 'CreditCard'){
    ...
    public function handleKnetResponse(Request $request, $isMobile = 0){
        ...
        $result_url = route('knetResponse');
        $result_params = "?PaymentID=" . $PaymentID . "&Result=" . $presult . "&PostDate=" . $postdate . "&TranID=" . $tranid . "&Auth=" . $auth . "&Ref=" . $ref . "&TrackID=" . $trackid . "&UDF1=" . $udf1 . "&UDF2=" .$udf2  . "&UDF3=" . $udf3  . "&UDF4=" . $udf4 . "&UDF5=" . $udf5  . "&isMobile=" . $isMobile;
    ...
    public function knetResponse(Request $request){
        ...
        if ( $presult == "CAPTURED" ){
            ...
            if(!empty($order))
            {
                ProductCart::where("user_id", "=", $order->customer_id)->delete();
                $cookie = \Cookie::forget('cookie.cartItem');
